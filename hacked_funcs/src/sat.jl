...
function slice_dfs(p::MaxSatProblem{INT, VT}, slicer::ContractionTreeSlicer, code::DynamicNestedEinsum{Int}, verbose::Int) where {INT, VT}
    ...
    n_clauses = length(initial_branch.p.clauses)

    finished_count = 0

    while !isempty(unfinished_slices)
        println("unfinished_slices: ", length(unfinished_slices))
        ...
        if cc.sc ≤ slicer.sc_target
            if branch_to_slice.p.use_constraint == true
                println("branch_to_slice.r: $(branch_to_slice.r), length(branch_to_slice.p.clauses): $(length(branch_to_slice.p.clauses)), n_clauses: $n_clauses")
                if branch_to_slice.r + length(branch_to_slice.p.clauses) == n_clauses
...
function _slice_single(slice::ST, slicer::ContractionTreeSlicer, size_dict::Dict{Int, Int}, verbose::Int) where ST
    ...
    println("new_slices: ", length(new_slices))
