...
function test_slice_dfs_lp(g, weights,sc_target)
    println("=" ^ 60)
    println("Testing slice_dfs_lp function")
    println("=" ^ 60)
    ...
    # Test slice_bfs
    println("\n" * "-" ^ 60)
    println("Running slice_dfs_lp...")
    println("-" ^ 60)
    
    try
        finished_slices = slice_dfs_lp(p, slicer, code, false, 1)
        ...
        if !isempty(finished_slices)
            ...
            println("Count maximum independent sets: ", mis_count_sum,", mis_size: $mis_size, mis_count: $mis_count")
            @assert mis_count_sum == mis_count "MIS count mismatch: expected $mis_count, got $mis_count_sum"
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    test_slice_dfs_lp(g, weights, 4)
    test_slice_dfs_lp(g, weights, 3)
