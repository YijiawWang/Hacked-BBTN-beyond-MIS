...
Parameterized driver for the **MIS ground-counting** branching benchmark
(`beyond_mis/hacked_funcs/benchmarks/mis_ground_counting.jl`). It can
either
...
Once the `(graph, weights)` is in hand, the script builds the initial
contraction code via `TreeSA`, then runs `slice_dfs_lp` (or `slice_bfs`
when `--no-lp` is passed) to decompose the contraction tree into slices
that fit a target space-complexity budget. Every finished slice is
written into `beyond_mis/branch_results/<subdir>/` using the streaming
writer from `beyond_mis/contractors/mis_slice_contract.jl`, so partial
progress survives a crash and the dump is immediately consumable by
`contract_mis_slices.jl`.

Defaults match `mis_ground_counting.jl`'s style (unit weights, KSG
density 0.8). The recognised `.graph` filename pattern is

    random_ksg_n=<n>_seed=<seed>.graph
        -> graph_type = `random_ksg`,
           subdir     = `mis_ground_counting_random_ksg_n=<n>_seed=<seed>`
...
Usage:
    ...
    julia --project=beyond_mis beyond_mis/scripts/_lib/slice_mis.jl \\
        ...
        [--weights=unit|random]  # vertex weights (default unit)
        [--weights-seed=<int>]   # seed for `random` weights (default = --seed)
        [--subdir=<name>]        # override the auto-generated slice subdir
        [--graph-type=<name>]    # override the auto-inferred graph_type tag
        [--no-lp]                # use slice_bfs instead of slice_dfs_lp (default lp)
        [--gpu=<id>]             # CUDA.device! before slice_dfs_lp (only used when --use-cuda)
        [--use-cuda]             # pass usecuda=true to slice_dfs_lp's solve_slice
...
`beyond_mis/branch_results/`; `contract_mis_slices.jl` consumes that
path next.
"""
...
# `hacked_funcs/src/mis_ground_counting.jl` and the
...
function _parse_args(args)
    ...
    for a in args
        ...
        elseif startswith(a, "--weights=")
            weights = String(split(a, "="; limit = 2)[2])
        elseif startswith(a, "--weights-seed=")
            weights_seed = parse(Int, split(a, "="; limit = 2)[2])
    ...
    weights in ("unit", "random") ||
        error("--weights must be 'unit' or 'random', got '$weights'")
    ...
    weights_seed === nothing && (weights_seed = seed)

    return MISSliceConfig(model_path, something(n, 0), sc_target, density,
                          seed, code_seed, weights, weights_seed,
                          subdir_override, graph_type_override, use_lp,
                          use_cuda, gpu_id, ntrials, niters, verbose)
end
...
function _build_problem(cfg::MISSliceConfig)
    ...
    if cfg.weights == "unit"
        ...
        rng = MersenneTwister(cfg.weights_seed)
        weights = abs.(randn(rng, Float32, nv(graph))) .+ 1.0f-3
    end

    meta["weights_kind"] = cfg.weights
    meta["weights_seed"] = cfg.weights_seed
...
function main(args)
    ...
    println("="^60)
    if !isempty(cfg.model_path)
        println("[slice_mis] model=$(cfg.model_path)  " *
                "sc_target=$(cfg.sc_target)")
    else
        println("[slice_mis] regenerate random KSG n=$(cfg.n) " *
    ...
    weights_suffix = cfg.weights == "random" ?
                     "_w=random$(cfg.weights_seed)" : ""
    subdir = isempty(cfg.subdir_override) ?
             "mis_ground_counting_$(base_id)$(weights_suffix)" :
             cfg.subdir_override
    ...
    if use_cuda && cfg.gpu_id >= 0
        try
            @eval Main using CUDA
            Main.CUDA.device!(cfg.gpu_id)
            println("  CUDA: bound to device #$(cfg.gpu_id)")
        catch err
            @warn "CUDA setup failed; falling back to CPU" exception = err
            use_cuda = false
        end
    end
    ...
    finished_slices = if cfg.use_lp
        slice_dfs_lp(p, slicer, code, use_cuda, cfg.verbose;
        ...
        slice_bfs(p, slicer, code, cfg.verbose;
            on_finished_slice = slice -> begin
                sid = append_mis_slice!(writer, slice; flush_summary = true)
                cc_s = complexity(slice)
                println("  [slice $sid saved] sc=$(cc_s.sc) tc=$(cc_s.tc) " *
