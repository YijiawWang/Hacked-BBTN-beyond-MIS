#!/usr/bin/env bash
#
# verify.sh — bash wrapper around `_lib/verify_mis_counting.jl`.
#
# End-to-end correctness driver for the MIS total-counting pipeline:
#   1) call slice_mis_counting.main(...) to slice and dump,
#   2) call contract_mis_counting_slices.main(...) to exactly contract,
#   3) build a fresh GenericTensorNetwork(IndependentSet(...)) and solve
#      CountingAll() to obtain the strict baseline,
#   4) assert (slice + sum) total IS count == strict total IS count.
#
# Usage:
...
JL_DRIVER="$SCRIPTS_DIR/_lib/verify_mis_counting.jl"
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<EOF
$(basename "$0") — verify the MIS total-counting slice → contract pipeline
...
Exits non-zero on count mismatch.
EOF
